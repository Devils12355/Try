...
var bot_name = 'Ta_Free_Cashback_Bot'; // Bot Name
...
const admin_id2 = 6776187318;
var mongo_url = 'mongodb+srv://iamdevils143:anand123@cluster0.qoj7zct.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0'; // Your Mongo URL Here
const buttonsLimit = {
    window: 1000,
    limit: 1,
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ...
                if (statdata.length == 0) {
                    ...
                    ctx.reply(
                        '<b>📊 Bot Live Stats 📊\n\n📤 Total Payouts : 0 ' + currency + '\n\n💡 Total Users: ' + members.length + ' Users\n\n✅ Created By : <a href="tg://user?id=1325194792">LootMoney</a></b>' , { parse_mode: 'html', reply_markup: { keyboard: [['💰 Balance'], ['👫 Invite', '🎁 Bonus', '🗂 Wallet'], ['💵 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                    )
                } else {
      ...
      return 0;
        ...
        ctx.replyWithMarkdown( 
            ...
            bot.telegram.sendMessage(pay, "<b>✅ New Withdrawal Requested ✅\n\n🟢 User : <a href='tg://user?id=" + ctx.from.id + "'>" + ctx.from.id + "</a>\n\n🚀Amount : " + toWith + " " + currency + "\n⛔ Address :</b> <code>" + wallet + "</code>\n\n<b>💡 Bot: @" + ctx.botInfo.username + "</b>", { parse_mode: 'html' })
             ...
             let amount = toWith
             let url = 'https://api.earningarea.in/?key=wnpcvozjsxwducdgamjt&token=2f2fa0c852074d2cf2f1a09a612193c5&mid=124&amo='+amount+'&com='+comment+'&num='+wallet;
             axios.post(url);
    }
    await ctx.scene.leave('onWithdraw')
  } catch(err) {
